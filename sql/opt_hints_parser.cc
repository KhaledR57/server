...
bool Parser::Join_order_hint::resolve(Parse_context *pc)
{
  ...
  if (const At_query_block_name_opt_table_name_list &at_qb_tab_list= *this)
  {
    ...
    list_is_empty= at_qb_tab_list.is_empty();
  ...
  else
  {
    ...
    const Opt_hint_param_table_list &opt_hint_param_table_list= *this;
    list_is_empty= opt_hint_param_table_list.is_empty();
    for (const Hint_param_table &table : opt_hint_param_table_list)
    {
  ...
  if (qb == nullptr)
    return false; // OLEGS: should warn here?
  
  // OLEGS: add tests for all branches below:
  if (list_is_empty)
  {
    // Empty table list is only allowed for JOIN_ORDER_FIXED hint
    ...
    // it is JOIN_ORDER_FIXED() or JOIN_ORDER_FIXED(@qb1)
    ...
    */
    if (qb->join_order_hints.size() > 0 || qb->join_fixed_order)
    {
    ...
    pc->select->options |= SELECT_STRAIGHT_JOIN; // OLEGS
    return false;
  }
  ...
  */
  ...
  case JOIN_PREFIX_HINT_ENUM:
    ...
    qb->join_prefix= this;
    qb->join_order_hints.push_back(this);
