...
void Opt_hints::print(THD *thd, String *str)
{
  ...
  for (uint i= 0; i < MAX_HINT_ENUM; i++)
  {
    ...
    if (is_specified(hint_type) && !is_ignored(hint_type))
    {
...
void Opt_hints_qb::apply_join_order_hints(JOIN *join)
{
  if (join_fixed_order)
  {
    ...
    // OLEGS: join->select_options |= SELECT_STRAIGHT_JOIN;
    return;
  }
  ...
  for (uint hint_idx = 0; hint_idx < join_order_hints.size(); hint_idx++)
  {
    ...
    if (set_join_hint_deps(join, hint, hint->hint_type))
    {
      // Mark as ignored
      join_order_hints_ignored |= 1ULL << hint_idx; // OLEGS: isn't overflow possible?
    }
  }
  ...
  // {
  //   join_prefix_applied=
  //       !set_join_hint_deps(join, join_prefix, JOIN_PREFIX_HINT_ENUM);
  //   // OLEGS: do we need join_prefix_applied member (and similar below) ?
  //   if (!join_prefix_applied)
  //     set_ignored(JOIN_PREFIX_HINT_ENUM);
  // }
  // if (join_suffix)
  // {
  //   join_suffix_applied=
  //       !set_join_hint_deps(join, join_suffix, JOIN_SUFFIX_HINT_ENUM);
  //   if (!join_suffix_applied)
  //     set_ignored(JOIN_SUFFIX_HINT_ENUM);
  // }
  // for (const Parser::Join_order_hint &join_order : join_order_hints)
  // {
  //   // OLEGS: temp soution, fix it for multiple JOIN_ORDERs
  //   join_order_applied=
  //       !set_join_hint_deps(join, &join_order, JOIN_ORDER_HINT_ENUM);
  //   if (!join_order_applied)
  //     set_ignored(JOIN_ORDER_HINT_ENUM);
  // }
}
...
{
  /*
    Make a copy of the original table dependencies. If an error occurs
    when applying the hints, the original dependencies will be restored.
  */
    ...
    for (uint i = 0; i < join->table_count; i++)
    {
      ...
      if (!compare_table_name(tbl_name_and_qb, table))
      {
        ...
        */
        ...
        hint_tab_map |= join_tab->table->map;
        break;
      }
    }
...
{
  String hint_name_str, hint_type_str;
  hint_type_str.append(opt_hint_info[type].hint_type); // OLEGS: MySQL renamed to hint_name which looks better
  append_table_name(thd, &hint_name_str, tbl_name.table_name, tbl_name.qb_name);
  uint err_code= ER_UNRESOLVED_TABLE_HINT_NAME; // OLEGS: fix this !
  push_warning_printf(thd, Sql_condition::WARN_LEVEL_WARN,
                      err_code, ER_THD(thd, err_code),
