...
sp_cursor *Sp_rcontext_handler_session::get_cursor_by_ref(THD *thd,
                                            const sp_rcontext_addr &ref,
                                            bool for_open) const
{
  Field *field= ref.rcontext_handler()->get_variable(thd, ref.offset())->field;
  return thd->m_session_cursors.get_cursor_by_ref(thd, field, for_open);
}
...
void sp_rcontext::sp_variable_destruct_all(THD *thd)
{
  if (m_var_table)
    m_var_table->sp_variable_destruct_all(thd);
}
